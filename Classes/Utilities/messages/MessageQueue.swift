class MessageQueue: NSObject {
	...
	//---------------------------------------------------------------------------------------------------------------------------------------------
	class func send(chatId: String, recipientId: String, text: String?, picture image: UIImage?, video: URL?, audio: String?) {
		...
		else if (image != nil)	{ sendMessageImage(message: message, image: image!)		}
	...
	//---------------------------------------------------------------------------------------------------------------------------------------------
	class func send(chatId: String, groupId: String, text: String?, picture image: UIImage?, video: URL?, audio: String?) {
	...
	class func forward(recipientId: String, dbmessage: DBMessage) {
		...
		if (dbmessage.type == MESSAGE_PICTURE)		{ forwardMessageImage(dbmessage: dbmessage, message: message)	}
	...
	//---------------------------------------------------------------------------------------------------------------------------------------------
	class func forwardMessageImage(dbmessage: DBMessage, message: FObject) {
		...
		if let path = DownloadManager.pathImage(dbmessage.objectId) {
			...
			ProgressHUD.showError("Missing media picture.")
		}
	}
	...
	//---------------------------------------------------------------------------------------------------------------------------------------------
	class func sendMessageImage(message: FObject, image: UIImage) {
		...
		if let data = image.jpegData(compressionQuality: 0.6) {
			DownloadManager.saveImage(message.objectId(), data: data)
			message[FMESSAGE_PICTUREWIDTH] = Int(image.size.width)
			message[FMESSAGE_PICTUREHEIGHT] = Int(image.size.height)
			createMessage(message: message)
		} else {
