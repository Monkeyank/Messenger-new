class DownloadManager: NSObject {
	...
	//---------------------------------------------------------------------------------------------------------------------------------------------
	class func startUser(_ name: String, pictureAt: Int64, completion: @escaping (_ path: String?, _ error: Error?) -> Void) {

		if (pictureAt != 0) {
			start(dir: "user", name: name, ext: "jpg", manual: false, completion: completion)
		} else {
	...
	//---------------------------------------------------------------------------------------------------------------------------------------------
	class func startImage(_ name: String, completion: @escaping (_ path: String?, _ error: Error?) -> Void) {
	...
	private class func start(dir: String, name: String, ext: String, manual: Bool,
		...
		if (manual) {
			if (File.exist(path: pathManual)) {
				completion(nil, NSError.description("Manual download required.", code: 101))
				return
			}
			...
			if let temp = try? String(contentsOfFile: pathLoading, encoding: .utf8) {
				if let check = Int(temp) {
					if (time - check < DOWNLOAD_TIMEOUT) {
						completion(nil, NSError.description("Already downloading.", code: 102))
						return
					}
				}
			}
